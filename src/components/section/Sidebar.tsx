...
export default function Sidebar() {
    ...
    // Mapping of paths to tab titles for reverse lookup
    ...
    const setTab = useCallback((tab: string) => {
        // Reverse mapping from Title to Path
        const titleToPath: Record<string, string> = Object.fromEntries(
            Object.entries(pathToTitle).map(([path, title]) => [title, path])
        );

        const path = titleToPath[tab];
        if (path) {
            navigate(path);
