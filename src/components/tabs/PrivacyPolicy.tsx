...
export default function PrivacyPolicy() {
    ...
    useEffect(() => {
        const fetchPolicy = async () => {
            try {
                const data = await systemService.getPrivacyPolicy();
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await systemService.updatePrivacyPolicy(content);
        ...
        <div className="w-full h-full p-6 flex flex-col bg-transparent overflow-hidden">
            <div className="bg-white dark:bg-card border border-gray-200 dark:border-gray-800 rounded-[2.5rem] p-8 shadow-xl flex flex-col flex-1 overflow-hidden relative">
                ...
                <div className="flex justify-between items-center mb-8 shrink-0 relative z-10">
                    <div>
                        <h1 className={"text-3xl text-gray-900 dark:text-white font-black tracking-tight"}>Privacy Policy</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Define and manage how user data is protected.</p>
                    </div>
                    <Button 
                        disabled={isSaving}
                        onClick={handleSave}
